...
class Visit
  ...
  def self.as_map(identifier)
    repository(:default).adapter.query("SELECT country_code, city, latitude, longitude, count(*) AS count FROM visits WHERE shortenedurl_urlshort = '#{identifier}' GROUP BY latitude, longitude")
  end
